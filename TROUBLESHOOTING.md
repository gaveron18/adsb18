...
### ПРОБЛЕМА: receiver-log не показывает Pi как активный без самолётов

**Симптомы:**
Окно «Лог работы приёмника» не показывает текущую сессию как активную,
хотя Pi онлайн и туннель работает.

**Причина:**
 проверял .
Если самолётов нет в зоне — позиции не пишутся в БД, проверка даёт False.

**Решение:**
 — curl к туннелю :30092, проверяет поле  < 30с.
Pi считается активным если туннель отвечает, независимо от наличия бортов.
Убрана также избыточная проверка  — она не нужна
когда  = реальная проверка туннеля. Коммит 65f1d3e.
...
**Причина:**
pi_active проверял max(ts) FROM positions за последние 5 минут.
Если самолётов нет в зоне — позиции не пишутся в БД, проверка даёт False.

**Решение:**
_check_pi_alive() — curl к туннелю :30092, проверяет поле now < 30с.
Pi считается активным если туннель отвечает, независимо от наличия бортов.
Убрана также избыточная проверка (now - end) < 10 мин — не нужна
когда pi_active = реальная проверка туннеля. Коммит 65f1d3e.
...
### ПРОБЛЕМА: `readsb-gain --help` ломает приёмник (gain=0, всё глохнет)

**Симптомы:**
- Запустили `sudo /usr/local/bin/readsb-gain --help` чтобы посмотреть синтаксис
- В `/etc/default/readsb` появилось `--gain --help` (вместо нормального значения)
- readsb принимает `--help` как **0.0 dB** — приёмник работает, но почти ничего не ловит
- В `journalctl -u readsb`: `rtlsdr: tuner gain set to 0.0 dB`
- В `stats.json`: `gain_db = 0.0`, `aircraft_with_pos = 0`

**Причина:**
Скрипт `/usr/local/bin/readsb-gain` от wiedehopf **не валидирует аргумент** —
делает `sed` подстановку напрямую: `sed -i "s/--gain [^ ]*/--gain $1/" /etc/default/readsb`.
Любая строка как `$1` подставится в конфиг и применится при следующем reload/restart.

**Как НЕ делать:**
- ❌ `sudo readsb-gain --help`  ← это меняет конфиг, не показывает справку
- ❌ `sudo readsb-gain auto-test` ← аналогично, попадёт в конфиг

**Как делать:**
- ✅ `sudo readsb-gain status` — показывает текущий gain
- ✅ `sudo readsb-gain <число>` — установить (например, `12.5`, `19.7`, `auto`)
- ✅ `sudo readsb-gain auto` — вернуть автоподбор
- Чтобы посмотреть что умеет скрипт: `cat /usr/local/bin/readsb-gain`

### ПРОБЛЕМА: близкие борта (rssi < −5 dBm) теряют ADS-B координаты

**Симптомы:**
- Дальние борта на rssi −20..−27 ловятся отлично, координаты приходят
- При близком пролёте (rssi > −5 dBm) борт виден только как `type: mode_s` без координат
- На FlightRadar этот же борт виден с трэком — то есть ADS-B Out у самолёта **есть**,
  это наш приёмник теряет DF17

**Причина (физическая, не программная):**
Clipping АЦП в RTL-SDR происходит **до** регулируемого VGA внутри тюнера R820T2.
Снижение `--gain` уменьшает усиление VGA, но **не входной мощности** на АЦП.
При rssi < −5 dBm АЦП насыщается, длинные DF17 (112 µs) повреждаются — CRC не сходится,
пакет отбрасывается. Короткие Mode-S (56 µs) выживают чаще → борт виден без координат.

**Подтверждение:** снижение gain с 22.9 до 12.5 (на 10.4 dB) изменило `peak_signal`
всего на 0.6 dB — потолок около −1 dBFS не двигается. См. `docs/session_2026-04-28.md`.

**Решение (только аппаратное):**
Поставить **RF-аттенюатор 6–10 dB** между антенной и RTL-SDR (например, VEGATEL AT-10 R00527).
После установки вернуть `--gain auto`.

**Текущий рабочий компромисс (без аттенюатора):**
`gain = 12.5` в `/etc/default/readsb` на Pi. Дальние ловятся хорошо (noise=−39 dBFS),
близкие при пиковых rssi теряют координаты — лечится только pad'ом.

**НЕ помогает:**
- Снижать gain дальше (12.5 → 0 даёт тот же peak, но дальние теряются)
- Менять PPM, `--adaptive-burst`, и прочее — overload физический
- Перенос антенны, экранирование — основная причина в близости громких бортов
