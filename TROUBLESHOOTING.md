...
## Сессия 2026-03-31 (зависание браузера после перезапуска API)

### ПРОБЛЕМА 19: Страница висит на загрузке — (pending) в DevTools

**Симптомы:**
- Браузер показывает бесконечную загрузку, серый экран
- DevTools → Network: единственный запрос (документ) со статусом `(pending)`, 0 B transferred
- Локально (`curl http://127.0.0.1:8098/`) страница открывается мгновенно

**Причина:**
После перезапуска `adsb18-api` (пока сервис поднимался ~3 секунды) nginx возвращал
`502 Bad Gateway`. Браузер держал keep-alive соединения открытыми. Когда API поднялся,
nginx попытался отправить ответы по старым TCP-сессиям — данные встали в kernel send buffer
и не дошли до браузера (TCP FIN-WAIT-1).

Диагностика через:
```bash
sudo ss -tnp | grep ':8098'
# Видно: ESTAB  send=106203  →  IP_браузера:PORT  (данные не уходят)
```
...
sudo nginx -s reload
```
Nginx сбросил зависшие соединения (FIN-WAIT-1), браузер открыл новые TCP-сессии.

**Правило:** После перезапуска `adsb18-api` — сразу делать `sudo nginx -s reload`,
чтобы сбросить зависшие keep-alive соединения браузеров.

---

### ПРОБЛЕМА 20: tar1090 "Problem fetching data — backend isn't working"

**Симптомы:**
- Страница открылась, но показывает красное сообщение об ошибке
- Карта пустая

**Причина:**
tar1090 фронтенд считает backend "сломанным" если получает 5+ ошибок подряд.
Это происходило во время ~3-секундного окна перезапуска API (502 ошибки).
Состояние ошибки сохраняется до перезагрузки страницы.

**Решение:** Ctrl+Shift+R (hard refresh) в браузере.
