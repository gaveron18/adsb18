...
async def send_to_server(queue: asyncio.Queue, server: str, port: int, name: str):
    """Reads from queue and forwards to adsb18 ingest server."""
    while True:
        try:
            log.info(f'Connecting to adsb18 server at {server}:{port}...')
            reader, writer = await asyncio.open_connection(server, port)
            log.info(f'Connected to adsb18 server')

            # Authenticate with feeder name
...
async def main(args):
    # Determine feeder name: use --name or hostname
    ...
    await asyncio.gather(
        ...
        send_to_server(queue, args.server, args.port, name),
    )
