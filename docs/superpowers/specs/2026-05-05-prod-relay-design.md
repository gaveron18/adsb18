...
```
            Pi (Ekaterinburg, T2 Russia, мобильный)
                       │
...
Шесть точечных артефактов, все обратимые.
...
1. Внести минимальную видимую правку (например `<title>tar1090 [DEV]`)
2. На dev открыть `http://173.249.2.184:8098/` — изменение видно
3. `git push origin main`
4. На prod `git pull` (как обычный деплой)
5. На `https://adsb18.ru/` — изменение тоже видно
6. Откат коммитом

Если этот цикл проходит — задача «восстановить dev для разработки» закрыта.
...
| # | Риск | Митигация |
|---|------|-----------|
| 1 | ТСПУ начнёт резать SSH с РФ-ЦОД к зарубежным IP — план рассыпается | Verification на этапе деплоя (см. 7.1 пункт 4). Если случится в эксплуатации — отдельный design (обфускация через VLESS-Reality на `buddy123`, либо перенос dev в РФ) |
...
| 4 | Pi выпадет из связи с prod на длительное время | dev и prod поллеры одновременно перестают писать — симметрично, никаких новых багов |
